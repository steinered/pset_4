...
If we look at all major/parent indicators, there are 38 Chi-Squared results greater than 3.841; therefore, there are 38 significant predictors across Tables 1-5.
...
```
The distribution of p-values is not normal; over half have a p-value < 0.05. There are some indicators which fall across the rest of the distribution, which may be indicative of some normal behavior; however, the majority of "significant" predictors are likely noise. 
...
```{r 4}
# bonferroni = p-value * m

m_all <- 65

df %<>% 
  mutate(bonferroni_all = (pval * m_all)) %>% 
  # filter bonferroni for under 0.05
  filter(bonferroni_all < 0.05)

# results 

# bonferroni for named indic only
m_named <- 65
df %>% 
  filter(grepl("I-", Indicators)) %>% 
  mutate(bonf_name = (pval * m_named)) %>% 
  filter(bonf_name < 0.05)
