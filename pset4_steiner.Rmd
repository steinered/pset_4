...
df %>% 
  ...
  arrange(-ChiSq) %>% 
  # mutate(
  # pval = cell_spec(formatC(pval, format = "e", digits = 2),
  #                  color = ifelse(pval > 0.05, "white", "black"),
  #                 background = ifelse(pval > 0.05, "red", "white"),
  #                 bold = ifelse(pval > 0.05, T, F))) %>% 
  # mutate(
  # bonf = cell_spec(formatC(bonf, format = "e", digits = 2),
  #                  color = ifelse(bonf > 0.05, "white", "black"),
  #                 background = ifelse(bonf > 0.05, "red", "white"),
  #                 bold = ifelse(bonf > 0.05, T, F))) %>% 
  # mutate(
  # qadjp = cell_spec(formatC(qadjp, format = "e", digits = 2),
  #                   color = ifelse(qadjp> 0.05, "white", "black"),
  #                 background = ifelse(qadjp > 0.05, "red", "white"),
  #                 bold = ifelse(qadjp > 0.05, T, F))) %>% 
  kable(col.names = c("Indicators", "Chi-Squared Statistic", "Original P-value", "Bonferroni Adjusted P-value", "BH Adjusted P-value"), caption = "Indicators and Corresponding Significance") %>% 
